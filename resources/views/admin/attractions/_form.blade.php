...
@endphp
    ...
    <div class="space-y-6">
        ...
        {{-- Name --}}
        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2">Attraction Name</label>
            <input type="text"
                   name="name"
                   value="{{ old('name', $attraction->name ?? '') }}"
                   class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[--primary-gold]/20">
        ...
        <div>
            ...
            <textarea name="description"
                      rows="4"
                      class="w-full px-4 py-3 rounded-xl border border-gray-200">{{ old('description', $attraction->description ?? '') }}</textarea>
        </div>
...
<script>
    window.addEventListener('load', function () {
        ...
        const map = L.map('admin-attraction-map', {
