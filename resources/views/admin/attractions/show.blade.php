...
@section('content')
    <div class="space-y-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.25em] text-[--primary-gold]">Attraction</p>
                <h2 class="text-3xl font-extrabold text-gray-800 mt-2">{{ $attraction->name }}</h2>
                <p class="text-gray-500 mt-2">Administrative detail view with map preview and content summary.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-6 rounded-3xl overflow-hidden bg-white border border-gray-100 shadow-xl">
                <div class="relative h-full min-h-[420px]">
                    <img src="{{ $attraction->image_url }}" alt="{{ $attraction->name }}" class="w-full h-full object-cover">
                    ...
                    <div class="absolute left-6 bottom-6 right-6 text-white">
                        <div class="flex flex-wrap gap-2">
                            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 border border-white/20 text-xs font-bold uppercase tracking-[0.2em]">
                                {{ optional($attraction->category)->name ?? 'Attraction' }}
                            </span>
                            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 border border-white/20 text-xs font-bold uppercase tracking-[0.2em]">
                                {{ number_format($attraction->reviews_avg_rating ?? 0, 1) }}/5
                            </span>
                        </div>
                        <h3 class="text-3xl font-extrabold mt-4">{{ $attraction->name }}</h3>
            ...
            <div class="lg:col-span-6 bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
                <div class="grid grid-cols-2 gap-4">
                    <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
                        ...
                        <p class="mt-2 font-bold text-gray-800">{{ optional($attraction->creator)->full_name ?? '-' }}</p>
                    </div>
                    <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
                        <p class="text-xs font-bold uppercase tracking-widest text-gray-400">Reviews</p>
                        <p class="mt-2 font-bold text-gray-800">{{ $attraction->reviews_count ?? 0 }}</p>
                    </div>
                    <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
                        <p class="text-xs font-bold uppercase tracking-widest text-gray-400">Rating</p>
                        <p class="mt-2 font-bold text-gray-800">{{ number_format($attraction->reviews_avg_rating ?? 0, 1) }}/5</p>
                    </div>
                    <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
                        <p class="text-xs font-bold uppercase tracking-widest text-gray-400">Coordinates</p>
                        <p class="mt-2 font-bold text-gray-800">{{ $attraction->latitude ?? '-' }}, {{ $attraction->longitude ?? '-' }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6 rounded-2xl border border-gray-100 bg-gray-50 p-5">
                    ...
                    <p class="mt-2 text-gray-700 leading-7">{{ $attraction->address ?? 'Location information unavailable.' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="admin-map-shell p-6">
            ...
            <div id="admin-attraction-show-map" class="w-full h-[420px] rounded-2xl overflow-hidden"></div>
        </div>
    </div>
    ...
    <script>
        window.addEventListener('load', function () {
            ...
            const map = L.map('admin-attraction-show-map', {
